# specs/ — bundled contracts for RAPP Test Neighbor
...
This directory was bundled on **2026-05-09T12:53:26Z** from `kody-w/RAPP @ fb784f9 (post-Bond-Pulse)`. The contracts here are FROZEN at planting time — they do not change unless an operator runs a `specs refresh` action.
...
| `SUBMISSION_PROTOCOL.md` | The kind-specific protocol — what `mirror` neighborhoods uniquely traffic in |
...
- **Lifted at:** 2026-05-09T12:53:26Z
...
```bash
# (future) brainstem run specs-refresh --neighborhood kody-w/rapp-test-neighbor
```
